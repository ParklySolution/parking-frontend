...
export default function WashServices() {
  ...
  const [drawerService, setDrawerService] = useState<WashService | null>(null);
  const [drawerPrice, setDrawerPrice] = useState<Partial<WashServicePrice> | null>(null);
  const [drawerEditService, setDrawerEditService] = useState<Partial<WashService> | null>(null);
  ...
  async function fetchAll() {
    ...
    try {
      const [svc, cat, prc] = await Promise.all([
        supabase.from("wash_service_catalog").select("*").eq("tenant_id", tenantId),
        supabase.from("vehicle_categories").select("*").eq("tenant_id", tenantId),
        supabase.from("wash_service_prices").select("*").eq("tenant_id", tenantId),
      ]);
  ...
  /* ======================================================
     ...
     ====================================================== */
  function openEditServiceDrawer(service: Partial<WashService> | null = null) {
  ...
  // Costruisci payload con TUTTI i campi della tabella
  const payload = {
    tenant_id: tenantId,
    name: form.name,
    code: form.code,
  ...
  try {
    ...
    if (form.id) {
      console.log("🔄 AGGIORNAMENTO servizio ID:", form.id);
      result = await supabase
    ...
    if (result.error) {
      console.error("❌ ERRORE DB DETTAGLIATO:", result.error);
      throw result.error;
    }
  ...
  /* ======================================================
     ...
     ====================================================== */
  if (!tenantId) {
    return <div style={{ color: "#ff4444", padding: "32px" }}>Errore: Tenant ID non trovato</div>;
  }

  if (loading) {
    return <div style={{ color: "#9ca3af", padding: "32px" }}>Caricamento servizi lavaggio...</div>;
  }
...
/* ======================================================
   FORM SERVIZIO (AGGIORNATO CON CAMPI MANCANTI)
   ====================================================== */
function ServiceForm({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <label style={label}>Codice *</label>
      <input
        style={input}
        value={form.code || ""}
...
const kpiLabel = { fontSize: "14px", opacity: 0.7 };
const kpiValue = { fontSize: "26px", fontWeight: 700, marginTop: "5px" };
...
const cardTitle = { fontSize: "20px", fontWeight: 700 };
const cardSubtitle = { opacity: 0.8 };
const cardDescription = { fontSize: "14px", opacity: 0.9 };
